...
export default function ProfileScreen() {
  ...
  const [followedPlayers, setFollowedPlayers] = useState<GolfPlayer[]>([]);
  ...
  useEffect(() => {
    ...
    golf.getFollowedPlayers(session.token).then(({ data: ids }) => {
      return golf.getPlayers().then(({ data: players }) => {
